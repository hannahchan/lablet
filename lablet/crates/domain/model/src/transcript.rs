...
//! One assistant message is one step; a step's tool results are in the user
//! message that follows it and join to its calls by call id; a step's metrics
//! are its [`TurnRecord`], where prompt tokens are `input_tokens`, completion
//! tokens are `output_tokens`, and cached tokens are `cache_read_tokens`.

use serde::{Deserialize, Serialize};

use crate::{ContentBlock, Message, Role, ToolCallId, Usage};
...
impl Transcript {
    ...
    /// Appends an assistant message made of `content` together with its
    /// record. Taking the content rather than a message keeps `turns` in step
    /// with the assistant messages whatever role the caller's message claimed.
    ...
    pub fn steps(&self) -> Vec<Step<'_>> {
        ...
        for (index, message) in self.messages.iter().enumerate() {
            ...
            let observation = self.messages.get(index + 1).filter(|next| {
                next.content
                    .iter()
...
impl<'a> Step<'a> {
    /// The [`ContentBlock::ToolResult`] block that answers the call `call_id`
    /// of this turn, if the tools ran.
    #[must_use]
    pub fn result_of(&self, call_id: &ToolCallId) -> Option<&'a ContentBlock> {
