...
/// under the id `call_n`.
fn completion(text: &str, tools: &[&str], finish: FinishReason, input: u64) -> Completion {
...
fn tool_turn(tally: &mut RunTally, tools: &[&str], statuses: &[ToolCallStatus]) {
    tally
        .completion(
            completion("On it.", tools, FinishReason::ToolUse, 100),
            ms(0),
            ms(10),
        )
        .unwrap();
...
fn finish(tally: RunTally, stop: StopReason) -> RunSummary {
    tally.finish(stop, ms(12_345), None, None, None).summary
}
...
fn the_prompt_sizes_are_byte_lengths_whether_or_not_a_turn_has_taken_the_prompt() {
    let mut tally = RunTally::start(setup(), "caf\u{e9}".to_owned(), "na\u{ef}ve".to_owned());

    let waiting = finish(tally.clone(), StopReason::Cancelled);
    tally
        .completion(
            completion("Hi.", &[], FinishReason::EndTurn, 1),
            ms(0),
            ms(1),
        )
        .unwrap();
    let taken = finish(tally, StopReason::Completed);
...
fn finish_writes_the_run_id_the_duration_in_whole_milliseconds_and_the_final_text() {
    ...
    tally
        .completion(
            completion("Partial answer.", &[], FinishReason::EndTurn, 1),
            ms(0),
            ms(1),
        )
        .unwrap();
    tally.failed_attempt(ms(1));

    let finished = tally.finish(
...
fn each_completion_is_a_turn_with_its_usage_and_finish_reason() {
    ...
    assert_eq!(tally.usage(), Usage::from_inclusive(280, 40, 0, 0));
    assert_eq!(tally.progress(ms(1_500)).turns, 2);
    assert_eq!(tally.transcript.turns().len(), 2);
    let finished = tally.finish(StopReason::Completed, ms(0), None, None, None);
...
fn the_returned_turn_is_the_one_the_transcript_now_ends_with() {
    ...
    let turn = tally
        .completion(
            completion("Hello.", &["bash"], FinishReason::ToolUse, 1),
            ms(40),
            ms(2),
        )
        .unwrap()
    ...
    assert_eq!(tally.transcript.turns(), [turn]);
}

#[test]
fn provider_latency_is_summed_and_its_maximum_kept_over_every_attempt() {
    let calling = completion("On it.", &["bash"], FinishReason::ToolUse, 1);
    let mut tally = start();
    ...
    let summary = finish(tally.clone(), StopReason::ProviderError);
    assert_eq!(summary.provider_latency_total_ms, 1_500);
    assert_eq!(summary.provider_latency_max_ms, 900);

    let answer = completion("Done.", &[], FinishReason::EndTurn, 1);
    tally.completion(answer, ms(0), ms(1_200)).unwrap();
...
fn a_turn_counts_the_attempts_of_its_call_and_the_next_call_starts_again() {
    ...
    assert_eq!(finish(tally, StopReason::Completed).provider_retries, 2);
}
...
fn a_retry_is_an_attempt_made_beyond_the_first_of_its_call() {
    ...
    tally
        .completion(
            completion("Done.", &[], FinishReason::EndTurn, 1),
            ms(0),
            ms(10),
        )
        .unwrap();
    ...
    let summary = finish(tally, StopReason::RetriesExhausted);
...
fn tool_calls_that_never_ran_are_in_no_total() {
    ...
    tally
        .completion(
            completion("Done.", &["task_complete"], FinishReason::ToolUse, 1),
            ms(0),
            ms(1),
        )
        .unwrap();
...
fn a_latency_is_truncated_as_it_is_recorded_so_totals_are_sums_of_whole_milliseconds() {
    ...
    tally
        .completion(
            completion("On it.", &["bash", "bash"], FinishReason::ToolUse, 1),
            ms(0),
...
fn a_completion_is_refused_while_the_last_turns_tool_calls_are_unanswered() {
    let mut tally = start();
    let calling = || completion("On it.", &["bash"], FinishReason::ToolUse, 1);
    tally.completion(calling(), ms(0), ms(1)).unwrap();

    assert_eq!(
        tally.completion(calling(), ms(0), ms(1)).unwrap_err(),
...
fn a_refused_completion_leaves_the_prompt_and_the_attempts_for_the_turn_that_is_accepted() {
    let mut tally = RunTally::start(setup(), "You fix tests.".to_owned(), "Go.".to_owned());
    tally.input.clear();
    tally.failed_attempt(ms(1));

    let refused = tally.completion(
        completion("Hi.", &[], FinishReason::EndTurn, 1),
        ms(0),
        ms(1),
    );
    ...
    tally.input = vec![UserContent::Text("Go.".to_owned())];
    let turn = tally
        .completion(
            completion("Hi.", &[], FinishReason::EndTurn, 1),
            ms(0),
...
fn a_completion_after_a_turn_that_called_no_tools_is_refused() {
    let mut tally = start();
    let answer = || completion("Done.", &[], FinishReason::EndTurn, 1);
    tally.completion(answer(), ms(0), ms(1)).unwrap();

    assert_eq!(
        tally.completion(answer(), ms(0), ms(1)).unwrap_err(),
...
fn outcomes_that_are_not_those_of_the_last_turns_calls_are_refused() {
    ...
    tally
        .completion(
            completion("On it.", &["bash"], FinishReason::ToolUse, 1),
            ms(0),
            ms(1),
        )
        .unwrap();
    ...
    assert!(tally.transcript.turns()[0].tool_calls().is_empty());
}
