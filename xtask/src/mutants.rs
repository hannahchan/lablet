...
pub fn check() -> CheckResult {
    ...
    // Under target/, which git ignores and `cargo clean` removes. cargo-mutants
    // does not create the parent of its output directory, and on a fresh clone
    // or a CI runner target/ does not exist yet.
    let output = workspace.root.join("target").join("xtask");
    std::fs::create_dir_all(&output)
        .map_err(|e| format!("could not create {}: {e}", output.display()))?;
    let output_arg = output.display().to_string();
    let mut args = vec!["mutants", "--output", &output_arg];
    for floor in FLOORS {
