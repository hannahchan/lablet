...
12. **Coverage and mutation testing on the core.** The domain crates and the application crate carry a 100% line and region coverage floor, and all three an 80% mutation score floor in CI. The loop reaches 100% because the run's states offer no way to write a sequence the transcript would have to refuse, so there's no defect check left that no test can reach. Regions are the stricter coverage measure: they count each match arm and each `else` a line never spells out apart, where a line counts as covered when any region on it ran. Adapters are held by conformance suites and recorded HTTP tests instead of a number.
