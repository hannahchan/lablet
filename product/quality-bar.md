...
11. **Contract-first telemetry.** Every span, event, attribute, and the wide event is declared in an OpenTelemetry Weaver registry before it is emitted. Rust attribute name constants, enums, and per-signal key lists are generated from the registry, the registry is checked for policy, and emitted telemetry is validated against it with `weaver registry live-check` in CI. Documentation is generated from the same source. An attribute cannot exist without being declared and documented.
12. **Coverage and mutation testing on the core.** Domain and application crates carry a 90% line coverage floor and an 80% mutation score floor in CI. Adapters are held by conformance suites and recorded HTTP tests instead of a number.
