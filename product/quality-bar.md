...
12. **Coverage and mutation testing on the core.** Domain crates carry a 100% line and region coverage floor, the application crate 98% of each, and all three an 80% mutation score floor in CI. The application crate's number is lower for one reason: the loop keeps checks against a defect of its own, which no test can reach and which stop it writing a transcript that lies about what happened. Regions are the stricter coverage measure: they count each match arm and each `else` a line never spells out apart, where a line counts as covered when any region on it ran. Adapters are held by conformance suites and recorded HTTP tests instead of a number.
