...
- `lablet-documents` under `crates/adapters/secondary/shared/`: the transcript document and the shapes it nests, deriving serde, built from the domain by a mapping that takes its source apart by pattern, so a new domain field is a compile error. It's a shared kernel because `provider-fake`'s script and the transcript writer both need `ContentBlock`, `ToolUse`, `ToolInput` and `Usage`, one to read and one to write. `Transcript`, `Turn` and the leaves they nest lose their derives when it lands; `serde_json::Value` stays in the domain, which is vocabulary rather than a wire form.
