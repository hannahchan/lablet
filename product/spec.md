...
| R     | finish reason `refusal`: `refused`. Finish reason `max_tokens`: `output_truncated`. Finish reason `context_window`: `context_exhausted`. All three hold whatever the response called, in both modes. Otherwise explicit mode and `task_complete` called with arguments that parse: `completed`, whatever else the response holds. Otherwise any tool call: the run goes on. Otherwise `completed` in natural mode and `ended_without_completion` in explicit mode |
...
| `explicit`          | The model calls the built-in `task_complete` tool, with arguments that parse, in a response with none of those three finish reasons. The call is intercepted by the loop, never executed, not counted in `tool_calls`, and its JSON argument is the run's structured result. Other tool calls in the same response are ignored. A `task_complete` call whose arguments don't parse completes nothing: it's answered as `malformed_input`, like any call with bad arguments, and the run goes on. | The model returns a turn with no tool calls. Stop reason `ended_without_completion`. |
...
- **Malformed provider responses** (the adapter can't map the payload to the domain model, including a tool call whose name or id isn't a valid `ToolName` or `ToolCallId`) count as retryable. A tool call whose arguments aren't valid JSON isn't one of them: the arguments are kept as the model wrote them, the call runs nowhere, and its outcome is `malformed_input`, an error result the model can correct from. Refusing the response instead re-rolls the same prompt against the same schema and reports a tool-surface problem as provider flakiness. That differs from a well-formed name that no tool has, which is an error result for the model. An OpenAI-compatible server that sends an empty or repeated call id gets a synthesised unique one from the adapter, not a `Malformed`.
...
When `run.transcript_path` is set, the whole conversation is written there as JSON when the run ends, whatever the stop reason. The document is `TranscriptDocument` (§3), rendered by the transcript writer adapter rather than by the model: `schema_version`, then the transcript's `system` and `turns`. The version is the document's and not the run's, which is why it's a separate type that borrows; `TRANSCRIPT_SCHEMA_VERSION` is 1, and it rises when a reader that knows the old form could misread the new one, which dropping a field, renaming one, or changing what one means all do and adding one doesn't. Each turn holds its `input`, which is the task prompt for the first turn and empty after that, since a run takes one prompt (the shape keeps room for a user who speaks again, which the loop doesn't offer yet); the model's `response`; the `record` of the provider call behind it (usage, finish reason, response id and model, when the call started as an offset from the start of the run, how long it took, and how many attempts it needed); and `tool_calls`, the outcome of each tool call the response made (call id, status, which says whether a tool ran and where it came from, start offset, latency, the size before the output cap when it cut, and the content the model was sent). A grader can therefore see a truncated or refused turn, which call failed and how, and where the time went, and the numbers agree with the run's totals because the totals are computed from them. A turn with no outcomes either called no tools or its tools never ran; its response says which. A run that received no response has no turns, so its transcript holds the system prompt alone. This is independent of telemetry and `capture_content`; it's what a grader lablet or a composing framework reads.
...
| outcome  | `lablet.run.stop_reason`, `error.type`, `lablet.run.error`, `lablet.run.duration_ms`, `lablet.run.turns`, `lablet.result.text_bytes`, `lablet.result.has_structured`, `lablet.run.transcript_path` (the path rendered as UTF-8, replacing anything that isn't)                                                                                                                                                                                                                                                                            |
...
// Transcript: private fields; built through Run, read through accessors, serialised through the document below and never read back
...
pub struct TranscriptDocument<'a> { pub schema_version: u32, pub system: &'a str, pub turns: &'a [Turn] }   // what a run emits; the version is the document's, not the run's
impl<'a> TranscriptDocument<'a> { pub const fn of(transcript: &'a Transcript) -> Self }   // borrows, so publishing copies nothing
impl Turn {
...
pub enum ProviderErrorKind { Retryable, ContextExhausted, Fatal, Malformed }   // as_str is the failed chat span's error.type; is_retryable() is Retryable or Malformed
...
pub struct RequestParams { pub max_tokens: u32, pub temperature: Option<f64>, pub thinking: Thinking, pub effort: Option<Effort>, pub seed: Option<i64> }
...
pub struct Prompts { system: String, task: String }   // one value, because two adjacent strings swap silently
impl Prompts { pub fn new(system: impl Into<String>, task: impl Into<String>) -> Result<Prompts, BlankTask>; pub fn system(&self) -> &str; pub fn task(&self) -> &str }   // a blank task is refused here, where it costs nothing; an empty system prompt is allowed
...
impl Run {
    pub fn start(setup: RunSetup, prompts: Prompts) -> Run;           // the task prompt waits as the first turn's input
...
impl Final {                                                    // a run whose last response called no tool can only finish
    pub fn turn(&self) -> &Turn; pub fn usage(&self) -> Usage;   // usage counts this turn, which the transcript doesn't hold until the run finishes
    pub fn finish(self, /* as Run::finish */) -> FinishedRun;
}
impl Pending {                                                  // finished at point R, or answered and carried on
    pub fn turn(&self) -> &Turn; pub fn usage(&self) -> Usage;
    pub fn calls(&self, mode: CompletionMode) -> Calls;         // what the stop policy reads at point R: TaskComplete exactly when completed_with is Some
    pub fn completed_with(&self, mode: CompletionMode) -> Option<&serde_json::Value>;   // explicit mode: the argument of the first task_complete call whose arguments parsed
    pub fn finish(self, /* as Run::finish */) -> FinishedRun;   // the turn's calls stay unanswered
    pub async fn answer<F, Fut>(self, schedule: Schedule<'_>, answer: F) -> Run where F: Fn(ToolUse) -> Fut, Fut: Future<Output = Answer>;
}
pub struct Schedule<'a> { pub max_concurrent: NonZeroU32, pub concurrency: &'a (dyn Fn(&ToolUse) -> ToolConcurrency + Sync) }   // what groups the calls; the loop reads a call's concurrency from its ToolSet
```
...
impl RetryPolicy {
    ...
    pub fn next(&self, attempt: u32, kind: ProviderErrorKind) -> Option<Duration>;   // the whole rule: retryable, and a retry left   // the wait after one-based attempt `attempt` failed; None when the call has had all its retries
}
...
run = Run::start(RunSetup { run_id, model, endpoint, tools.specs(), tools.completion(), max_turns, timeout, request }, prompts)
loop:
    if cancelled: stop cancelled;  if let Some(r) = stop.before_call(&run.progress(elapsed)): stop r
    attempt = 1
    (response, began, latency) = loop:                                     # one provider call
    ...
    run = p.answer(Schedule { max_concurrent: max_concurrent_tool_calls, concurrency: |call| tools.concurrency(&call.name) }, async |call|:   # the name is resolved before the arguments are read
        ...
        (status, content, mcp) = match tools.source(&call.name):        # the loop is the only hop from executor to observer, so mcp travels with the result
            ...
            Some(source)  -> match call.input:
                Unparsed(text) -> (MalformedInput, [Text("the arguments weren't valid JSON ...")], None)
...
`ToolSet` is a composite over several `ToolExecutor`s in this crate that routes a call by name, applies the config allow and deny lists, and rejects duplicate names at build time. It's deliberately not a `ToolExecutor` itself: the loop needs `source(&name)`, which the port can't answer. It also holds the run's `CompletionMode`, the one value whose shape depends on it, and everything else that needs the mode reads it back from there. The `task_complete` tool spec is defined in this crate, registered by `ToolSet` only in `explicit` mode regardless of `tools.builtin.enabled`, and never executed: the loop intercepts it. Its name is claimed before any executor is asked, so an executor serving `task_complete` in `explicit` mode is a duplicate-name error rather than a tool offered twice. Its input schema is `run.completion_schema` when set, otherwise a free-form object. `ToolSet` is where "what happens when a tool is missing" is expressed: the tool is simply absent from `specs()`.
...
Plays a scripted sequence of `ProviderResponse`s from a YAML or JSON file, read into the domain through `ProviderResponse::new` by this adapter's own types (§3), with optional per-call latency (real time) and injected errors of each `ProviderError` class. Selected with `model.provider: fake` and `model.script: path`. Reports usage from the script so token accounting is exercised end to end. Used by lablet's smoke tests, doctests, and examples, and by users testing their own frameworks. `lablet init --provider fake` writes both a config and a script.
...
| ProviderCallFailed      | `gen_ai.client.operation.exception` log record (severity WARN) in the chat span's context, plus a span event on the chat span for the retry | log record: `exception.type` (the `ProviderError` variant name, as `error.type` on the span), `exception.message`, the join keys, `gen_ai.operation.name`, `gen_ai.provider.name`, `gen_ai.request.model`, `lablet.turn`, `lablet.attempt`; span event `lablet.retry`: `lablet.attempt`, `lablet.retry.will_retry` and `lablet.retry.backoff_ms`, both read from the event's one `retry` field. The deprecated `exception` span event isn't used                                                                                                                                                                          |
...
A `Lablet` may run many times; each `run` gets a fresh `RunId` and `RunContext`, MCP servers persist across runs, and `shutdown` is the only teardown. `run` takes the `FinishedRun` that `RunService::run` returns, writes its transcript to `run.transcript_path` when that's set, and returns its outcome. The wide event is emitted inside `RunService::run`, so it goes before the write: `lablet.run.transcript_path` names where the transcript is written rather than reporting a file that already exists. A write that fails is reported on the diagnostic log and leaves the outcome alone, as an observer's export failure does (§5); the run measured what it measured either way. Runs on one `Lablet` are sequential.
...
- A schema version field on the outcome JSON. Deferred until that schema settles; until then a reader tells the forms apart by their keys. The transcript document has one, since `TranscriptDocument` arrived with the version on it.
