...
pub enum ToolCallStatus { Ok, ToolError, Unknown, Timeout, Failed }   // as_str is `lablet.tool.status`, and the span's error.type for every value but ok; is_error() is status != Ok
