...
pub struct ProviderResponse { content: Vec<ContentBlock>, pub usage: Usage, pub finish: FinishReason, pub response_id: Option<String>, pub response_model: Option<String> }
...
**Empty text.** When a completion becomes a turn, `Text` blocks that are empty or only whitespace are dropped from the response. This is the one place the transcript differs from what the provider sent. Models do emit such blocks, typically just before a tool call, and Anthropic refuses both kinds when the response is replayed; rejecting the completion instead would mark it `Malformed` and spend a retry on a block that carries nothing. The drop happens once, before anything reads the turn, so `final_text()` and every byte count taken from `Run::messages()` describe what's replayed.
