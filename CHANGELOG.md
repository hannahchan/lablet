...
- Transcript: the transcript document is now `system`, `prompt`, and `turns`. Each turn holds the model's response, a record of the provider call behind it (usage, finish reason, start offset, latency, attempts), and the outcome of each tool call it made (status, source, start offset, latency, truncation, and the content the model was sent).
