...
- Telemetry contract: `lablet.run.stop_reason` gains the member `refused`. The `lablet.run` wide event gains `server.address` and `server.port`, present when the provider is reached over the network, and `lablet.tool_calls.unknown`, the calls to a tool name the run didn't offer; such a call counts in the totals and gets no `lablet.tool.*.<name>` key, so those keys stay bounded by the configured tools. `lablet.provider.retries` now counts the attempts made beyond the first of a call, so a run whose only provider call fails reports none. On the `lablet.execute_tool` span, `gen_ai.tool.type` and `lablet.tool.source` are required only for a tool the run offered. `gen_ai.request.reasoning.level` follows `model.effort`, and the note of `lablet.request.bytes` says what's measured.
