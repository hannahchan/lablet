...
Phase 3 inherits one: if resolving a name through the `ToolSet` is the only way to obtain a `ToolSource`, a tool that appears mid-run can't produce a `Ran` outcome at all, and the per-tool bound stops depending on an adapter's good behaviour.
...
11. **Does a constant live in prose and nowhere in code?** `6b4d0d3` found `TASK_COMPLETE` and `ToolName::MAX_LEN` written in the spec and declared nowhere.
...
17. **Reject rather than model, when the modelling costs more than the errors it deletes.** The typestate refusal above is the case (`2a575f8`).

## Tests

`6b4d0d3` is the richest commit here.

18. **Does the test assert both halves of its name?** `a_refused_completion_leaves_the_prompt_and_the_attempts...` never checked the prompt: it cleared the input, took the refusal, then reassigned the input by hand, so clearing it on the error path would have passed.
...
24. **Does a doc claim a mechanical property that isn't mechanical?** `spellings.rs` claimed each match is exhaustive; it held for two of the four, and the others were array literals, so a new `ToolSource` variant would have compiled, passed conformance, and emitted a value the registry has no member for.
...
27. **Open the documentation.** The domain modelled a provider API from memory, no adapter existed to contradict it, and no gate could (`b1eff3d`). One page read found three defects, including a request shape that returns 400 on every current model.
...
31. **Report rather than withhold.** `Pricing::cost` prices a self-contradicting usage low rather than refusing: refusing is the worse failure for a tool whose job is to report what happened, and the raw counts reach the wide event beside it.
...
34. **Structural change is priced by the number of dependants.** Zero is the moment. Five commits carry the argument, in the form "before the loop is written against them."
...
37. **What stays:** those five, and anything that's the only record of a fact.
...
39. **One module per idea, not one per stage of a run.** Name modules on one axis. `lablet-model` used two, so anything fitting neither landed in the nearest stage-module and `provider.rs` and `outcome.rs` became catch-alls.
...
51. **Record a decline with a reason that stays valid.** Splitting `message.rs` was declined because `Message<'a>` borrows from the owned types, so a module line would make the lifetime harder to follow. Splitting the large test files was declined as navigability alone, which sets the bar.
52. **Re-measure the floors after pure movement.** A move that drops a test file, or leaves a `mod tests;` pointing at nothing, shows up there and nowhere else.
...
- **Mutation testing doesn't generate every narrowing.** Restricting the per-tool branch to `Builtin` is a change no mutant proposes (item 20).
...
- **A doc comment claiming a mechanical property isn't itself mechanical** until something fails to compile (item 24).
