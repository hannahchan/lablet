...
- Domain crates contain types and pure functions only: no runtime, no I/O, no clock, no port, no threads. The test is that a plain `#[test]` drives every domain function deterministically. An async function passes when its tests poll it to completion with `std::task::Waker::noop()` and futures the test controls, as `Pending::answer`'s do. Anything that needs a runtime to test is application or outward.
...
CI runs on every pushed branch as a matrix of `cargo xtask ci` (the pre-push list), `cargo xtask coverage` (floors: every line and every region on `lablet-model` and `lablet-policy`, 99% of each on `lablet-run`), and `cargo xtask mutants` (floor: 80% caught on the same crates). The domain crates are held to 100% because a branch no test can take is the signal these floors exist to raise: the type allows a state the caller has already ruled out. `lablet-run` is held at 99, which is where it stands, because it can't reach 100: two arms take a `TranscriptError` the loop can't receive, since the rules that error reports are ones the loop maintains as it goes. Those checks are a third kind of uncovered code, neither of the two the domain floors are set for. A guard against a state the caller has already ruled out should go, and the type should rule it out instead; a shared contract, where a second caller can genuinely fail the check, stays and is covered by that caller's tests. These are neither, and what they buy is that a future defect ends the run with a reported `provider_error` rather than a transcript that lies about what happened. There's no headroom in the regions at all, so one new region no test can reach turns the gate red on the commit that adds it, which is the point: a check of that third kind should be argued for when it's written rather than discovered later. No crate is held to catching every mutant: an equivalent mutant can't be killed by any test, and `lablet-run` carries one. A region is a span of source with its own counter, so a match arm, an `else` a line never spells out, and the far side of a `&&` are each counted apart; a line is covered when any region touching it ran, which makes regions the stricter of the two and 100% regions imply 100% lines. Branch coverage is finer again in the other direction, since it alone sees a condition that was never false, but it needs a nightly toolchain and this workspace pins a stable one. A floor crate may measure nothing only while its `src/` defines no function; after that, a run with no lines, no regions, or no mutants for it fails. A trait method without a body counts as a function, so the first function with a body lands with its test in the same push as the first port. Later phases add `cargo xtask weaver live-check` (phase 6), `cargo xtask bench` with a 20% regression threshold, and the release builds (phase 11). `xtask` is a root-level crate reached through the alias, which is defined twice, in `.cargo/config.toml` and `lablet/.cargo/config.toml`; an xtask test keeps the two in step. It doesn't work from a crate directory below `lablet/`.
